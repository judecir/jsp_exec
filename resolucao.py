...
def teste_output_christophe():
    ...
    lista_modelos = [jsp_disjuntivo_manne, jsp_disjuntivo_minla_favorito]
    df = execucao_modelos([i for i in lista_instancias if i["id"] =='la01']
                          ,lista_modelos
                          ,prefixo_arq="t_sol"
                          ,tempo_max=3600
                          ,fl_primeira_sol=False)
...
def execucao_oficial():
    instancias = criar_instancias()
    
    
    nomes_arquivos = os.listdir(os.getcwd()+"/"+"Output_christophe_heuristica")
    #filtro das instancias de taillard
    nomes_arquivos = [i for i in nomes_arquivos if i!= "desktop.ini" and 'yn' not in i]
    nomes_arquivos.sort()
    
    for i in nomes_arquivos:
        ind = find(instancias, "id",i)
        if ind>0:
            ub = int(i.replace(".txt", "").replace(instancias[ind]["id"]+"-", ""))
            instancias[ind]["lista_ub"].append(ub)
    ...
    # lawrence
    indices = indices + [i for i in range(10, 30)]
    # taillard
    indices = indices + [i for i in range(80, 100)]
    for i in indices:
        lista_instancias.append(instancias[i])
